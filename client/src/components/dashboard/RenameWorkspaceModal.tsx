...
import { updateWorkspace } from "@/lib/workspaceService";
...
export default function RenameWorkspaceModal({ workspace, onClose, onUpdated }: RenameWorkspaceModalProps) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Could not update the workspace. Please try again.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <Modal open={workspace !== null} onClose={handleClose} labelledBy="edit-workspace-title">
      <div className="p-6 sm:p-7">
        <div className="mb-6">
          <h2 id="edit-workspace-title" className="text-lg font-semibold text-white">Edit Workspace</h2>
          <p className="mt-1 text-sm text-white/50">Update the details of this workspace.</p>
        </div>
